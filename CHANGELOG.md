...
**UI Overhaul — Sidebar Navigation, Library Panel, Alpine.js Frontend.**

v5 is a complete frontend rewrite. The backend is unchanged — no database migrations, no config changes, no API changes. The upgrade is a container restart.

**Navigation — Tab bar replaced with sidebar**

- Horizontal tab bar removed. Navigation is now a fixed sidebar on the left with three groups: Monitor (Sweep, Library, Intel), Configure (Instances, Pipelines, Overrides, Filters, Settings, Notifications), and System (Advanced).
- Overrides and Filters are sub-items under Pipelines in the sidebar, indented to reflect their relationship.
- Unsaved changes indicator: a small amber dot appears next to any sidebar nav item with pending saves. Navigate freely between tabs without losing edits.
- Hamburger overlay at ≤720px: sidebar slides in from the left as an overlay with a dark backdrop. Tap outside or tap any nav item to dismiss.

**Library Panel — History, Imports, CF Score, and Exclusions unified**

- Four separate tabs (History, Imports, CF Score, Exclusions) are now one Library panel with a view switcher.
- Exclusions get their own dedicated view with full filtering, sorting, and pagination. Previously embedded in History.
- Library nav item shows a badge count for unacknowledged auto-exclusions, clearing when the Exclusions view is opened.

**Pipelines Panel — Pipeline config centralised**

- Backlog and CF Score configuration moved from Advanced and Settings into a new Pipelines tab.
- Pipelines sits alongside Cutoff Unmet config so all three pipeline settings are in one place.
- CF Score scan controls (Scan Library, Reset Index, coverage list, last synced / next sync) moved into Pipelines.

**Settings restructured**

- Settings now covers Schedule, Sweep Controls (Cooldown, Queue Depth, Per-Instance Overrides toggle), Throttling (Batch Size, Sleep, Jitter), and Auto-Exclusion.
- Maintenance Window renamed to Quiet Hours throughout.
- Queue Depth moved from Advanced into Settings → Sweep Controls.

**Topbar status strip**

- Scheduler status, last run, next run, and Run Now button are now persistent in the topbar on every panel.
- Smart time formatter: displays "2h ago", "3d ago", "Apr 9" depending on the interval. Next run shows green when scheduled, amber when overdue, muted when manual mode.
- Manual mode hides the next run segment entirely — no redundant "Manual" label.
- Run Now guards against no instances configured with an inline empty state on Sweep and a modal prompt from the topbar.

**Frontend architecture**

- Vanilla JS split (12 files) replaced with a single Alpine.js v3 data object (app.js).
- 15 template partials replaced with a single ui.html.
- Alpine.js v3.14.9 self-hosted alongside fonts — no CDN dependency.
- Tab migration guard: existing `default_tab` values of `history`, `imports`, `cf-scores` are transparently migrated to `library` on upgrade. Applied both in config.py on load and in the frontend on startup.
- `TAB_MIGRATION_V5` constant added to constants.py. Applied in config.py validation so existing installs upgrade cleanly without error.
- VALID_TABS updated: `library` and `pipelines` added, `history`, `imports`, `cf-scores` removed as standalone values.

**Modals and flows**

- Alert modal is now typed: error (red tint) and success (green tint), no icons, title-cased titles.
- What's New modal updated to describe v5 UI changes accurately.
- Onboarding walkthrough fully updated for v5: sidebar navigation references, Library → History / Library → Imports, Pipelines for backlog and CF Score config, Run Now in topbar.
- Overrides Info modal wording updated for v5 tab structure.

**Login and Setup pages**

- Both pages restyled with v5 palette, Outfit font, and Nudgarr gradient wordmark.
- Setup page help text updated: "Advanced → Security" (was "Advanced → Require Login").

**No backend changes**

- All 29 API endpoints unchanged.
- Database schema unchanged, no migration required.
- All config keys unchanged.
- Existing history, exclusions, imports, CF Score index, and Intel data are untouched.
