...
function lsOvRenderRail() {
  ...
  ['radarr', 'sonarr'].forEach(kind => {
    (CFG.instances?.[kind] || []).forEach((inst, idx) => {
      ...
      const dotGlow = (!isDisabled && isActive) ? (kind === 'sonarr' ? ';box-shadow:0 0 4px rgba(34,197,94,.5)' : ';box-shadow:0 0 4px rgba(91,114,245,.6)') : '';
      ...
      html += '<div class="ls-ov-rail-item' + (isActive ? ' ls-ov-active' : '') + (isDisabled ? ' ls-ov-disabled' : '') + '"'
        ...
        + '<span style="width:5px;height:5px;border-radius:50%;background:' + dotColor + dotGlow + ';flex-shrink:0;display:inline-block;margin-right:4px"></span>'
...
function lsOvRenderPanel() {
  ...
  const panelName = document.getElementById('ls-ov-panel-name');
  if (panelName) {
    const badge = kind === 'radarr'
      ? '<span class="ls-ov-panel-badge">Radarr</span>'
      : '<span class="ls-ov-panel-badge ls-ov-panel-badge-sonarr">Sonarr</span>';
    panelName.innerHTML = escapeHtml(inst.name) + ' ' + badge;
  }
