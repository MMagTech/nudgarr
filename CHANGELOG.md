...
- **Security:** Pin `urllib3` to 2.7.0 (fixes CVE-2026-44431, CVE-2026-44432).
...
- **Library Imports:** Turnaround column is sortable (uses numeric `turnaround_days`). Title **`search`** query param wired through `GET /api/stats` to `get_confirmed_entries()` (`LOWER(COALESCE(se.title,'')) LIKE` for null-safe match). Library title search on History / Imports / CF Score / Exclusions uses **`_scheduleLibrarySearchRefresh()`** (explicit `setTimeout` debounce) so refresh always runs after `x-model` has the latest text — Alpine’s `@input.debounce` alone could still race the model update. Search query strings are trimmed before calling the API.
